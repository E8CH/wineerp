...
> **추가 경위(2026-07-21)**: 구현 단계에서 드러난 두 가지 계획 공백을 정식화한다.
> ① 라벨 사진이 서버에 영속 저장·조회되지 않아 실기에서 아이콘으로만 보였다(FR4가
> "첨부·표시"를 요구하나 저장/조회 경로가 에픽에 없었다). ② UX-DR3가 하단 4탭에
> **재고** 탭을 정의하고 앱 셸에도 탭이 있으나, 어느 FR에도 걸리지 않아 어떤 스토리도
> 이를 구현하지 않았다(IR의 FR 커버리지 100% 검증이 UX가 만든 탭은 짚지 못한 사각지대).
> 계획에 없던 것을 조용히 만들지 않기 위해 여기에 등재한다.

### Story 6.1: 라벨 사진 저장·조회 (FR4, AR4)

As a 직원,
I want 촬영한 라벨 사진이 저장되고 이후 화면에서 다시 보이기를,
So that 입고 확인·내역·재고에서 병을 사진으로 식별한다.

**Acceptance Criteria:**

**Given** 라벨 사진을 촬영·업로드한 뒤
**When** 확인 카드·입고 내역·재고 목록을 열면
**Then** 저장된 사진이 인증된 경로로 조회되어 표시되고, 사진이 없는 와인은 병 아이콘으로 폴백된다.
**And** 사진은 재배포에도 유실되지 않게 서버에 영속 저장되며(로컬 디스크 금지), 조회는 인증 필수·기기 캐시로 반복 조회 비용을 줄인다.

### Story 6.2: 재고 목록 (UX-DR3)

As a 직원,
I want 보유 와인의 빈티지별 현재고를 재고 탭에서 보기를,
So that 어떤 와인이 몇 병 있는지 한눈에 확인한다.

**Acceptance Criteria:**

**Given** 재고 탭에서
**When** 화면을 열면
**Then** 빈티지(재고 단위)별로 생산자·모델명·빈티지·라벨 사진·현재고가 목록으로 표시된다.
**And** 현재고는 서버 집계(입고 합계, soft-delete 제외, 초기 세팅분 포함)를 그대로 쓰고, 재고 0인 등록 와인도 숨기지 않으며, 입고·등록 후 재고가 갱신된다. NV는 오류가 아닌 유효 상태로 표기한다.
